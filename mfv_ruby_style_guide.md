...
* <a name="same-line-condition"></a>
  Always put the condition on the same line as the `if` in a
...
<sup>[[link](#no-and-or-or)]</sup>
  ...
  # control flow
  fail(RuntimeError, "Failed to save document!") if !document.save
...
* <a name="if-as-a-modifier"></a>
  Favor modifier `if` usage when you have a single-line body. Another
...
* <a name="no-multiline-if-modifiers"></a>
  Avoid modifier `if` usage at the end of a non-trivial multi-line
  block.
...
* <a name="no-nested-modifiers"></a>
  Avoid nested modifier `if`/`while`/`until` usage. Favor `&&`/`||` if
  appropriate.
...
* <a name="never-use-unless"></a>
  Never use unless. Even if your code looks more simple and clearer now, it is 
  going to change a lot in the future. Usually, other people will just add more
  conditions without replacing `unless` with `if`, and the code becomes messy
  quickly. 
<sup>[[link](#never-use-unless)]</sup>
